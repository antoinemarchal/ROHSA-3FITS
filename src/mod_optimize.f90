...
module mod_optimize
  ...
  public :: gaussian, myfunc_spec, myresidual, mygrad_spec, f_g_cube_fast, f_g_cube_fast_norm
...
contains
 ...
 ! Compute the objective function for a cube and the gradient of the obkective function
  ...
  ! ! Compute the objective function for a cube and the gradient of the obkective function
  ! subroutine f_g_cube_fast_new_var(f, g, cube, beta, dim_v, dim_y, dim_x, n_gauss, kernel, lambda_amp, lambda_mu, lambda_sig, &
  !      lambda_var_amp, lambda_var_mu, lambda_var_sig, std_map)
  !   implicit none
  ...
  !            f = f + (0.5_xp * lambda_amp * conv_amp(j,l)**2)
  !            f = f + (0.5_xp * lambda_mu * conv_mu(j,l)**2)
  !            f = f + (0.5_xp * lambda_sig * conv_sig(j,l)**2) + (0.5_xp * lambda_var_sig &
  !                 * (image_sig(j,l)/b_params(i) - 1._xp)**2._xp)
             
  !            g((n_beta-n_gauss)+i) = g((n_beta-n_gauss)+i) - (lambda_var_sig * 1/b_params(i)**2._xp &
  !                 * (image_sig(j,l)/b_params(i) - 1._xp))        
  ...
  !            deriv(2+(3*(i-1)),j,l) = deriv(2+(3*(i-1)),j,l) + (lambda_mu * conv_conv_mu(j,l))
  !            deriv(3+(3*(i-1)),j,l) = deriv(3+(3*(i-1)),j,l) + (lambda_sig * conv_conv_sig(j,l) + &
  !                 (lambda_var_sig * 1._xp/b_params(i) * (image_sig(j,l)/b_params(i) - 1)))
  !         end do
  !         !
  !      end do
  !   end do        
  ...
  ! end subroutine f_g_cube_fast_new_var
